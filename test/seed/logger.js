(function() {
  ...
  QUnit.module('mob/logger', {
    beforeEach: function() {
      var calls = [];

      this.calls = calls;
      this.logger = Mob.Logger;
  ...
  QUnit.test('Mob Logger - Log messages routed to logger function', function(assert) {
  ...
  QUnit.test('Mob Logger.enabledFor', function(assert) {
    var logger = this.logger;
  ...
  QUnit.test('Named logger messages not routed to global logger', function(assert) {
    ...
    logger.setLevel(Mob.Logger.DEBUG);
    namedLogger.setLevel(Mob.Logger.DEBUG);
  ...
  QUnit.test('Named logger setLevel does not affect global logger', function(assert) {
    ...
    this.logger.setLevel(Mob.Logger.OFF);
    named.setLevel(Mob.Logger.DEBUG);

    named.debug('Debug message via nemd logger');
    this.logger.debug('Debug message via Mob Logger');
  ...
  QUnit.test('Logger.log convenience method', function(assert) {
    this.logger.setLevel(Mob.Logger.INFO);
    this.logger.log('log message');

    assert.ok(this.calls.length === 1);
    assert.strictEqual(this.calls[0].context.level, Mob.Logger.INFO, 'Logger.log message routed at INFO level');
  });

  QUnit.test('Named logger.log convenience method', function(assert) {
    var named = this.logger.get('logHelper');
    named.setLevel(Mob.Logger.INFO);
  ...
  QUnit.test('Logger.useDefaults can be supplied a custom message formatter', function(assert) {
    ...
    assert.ok(formatterSpy.firstCall.args[1].name === 'Dave', 'Context passed to formatter');
  });
