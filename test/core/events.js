(function() {
  ...
  QUnit.test("listenTo and stopListening", function(assert) {
    assert.expect(1);
    var a = Mob.extend({}, Mob.Events);
    var b = Mob.extend({}, Mob.Events);
  ...
  QUnit.test("stopListening cleans up references", function(assert) {
    ...
    assert.equal(Mob.size(a._listeningTo), 0);
    assert.equal(Mob.size(b._events.event), 1);
    assert.equal(Mob.size(b._listeners), 0);
  ...
  QUnit.test("listenToOnce with event maps cleans up references", function(assert) {
    ...
    b.trigger('one');
    assert.equal(Mob.size(a._listeningTo), 1);
  });
  ...
  QUnit.test("once variant one", function(assert) {
    ...
    var a = Mob.extend({}, Mob.Events).once('event', f);
    var b = Mob.extend({}, Mob.Events).on('event', f);
  ...
  QUnit.test("once with asynchronous events", function(assert) {
    ...
    assert.expect(1);
    var func = Mob.debounce(function() {
      assert.ok(true);
      done();
    }, 50);
    var obj = Mob.extend({}, Mob.Events).once('async', func);
  ...
  QUnit.test("once without a callback is a noop", function(assert) {
    assert.expect(0);
    Mob.extend({}, Mob.Events).once('event').trigger('event');
  });
  ...
  QUnit.test("event functions are chainable", function(assert) {
    var obj = Mob.extend({}, Mob.Events);
    var obj2 = Mob.extend({}, Mob.Events);
    var fn = function() {};
  ...
  QUnit.test("#3448 - listenToOnce with space-separated events", function(assert) {
    assert.expect(2);
    var one = Mob.extend({}, Mob.Events);
    var two = Mob.extend({}, Mob.Events);
    var count = 1;
